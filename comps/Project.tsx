...
const Project = () => {
    ...
    return(
        <Element name="Project" className='relative py-12 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8 lg:px-16 animated-bg'>
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className='mb-8 sm:mb-12'>
                    ...
                    <div className="text-center mb-4 sm:mb-6">
                        <button 
                            className={`px-4 sm:px-6 py-2 sm:py-3 rounded-full text-sm font-medium transition-all duration-300 transform hover:scale-105 ${
                                finding_tag === "" 
                    ...
                    <div className="relative max-w-md mx-auto">
                        <button 
                            className="w-full px-3 sm:px-4 py-2 sm:py-3 bg-background-300 border border-gray-600 rounded-lg text-gray-300 hover:border-purple-500 hover:text-purple-400 transition-all duration-300 flex items-center justify-between text-sm sm:text-base"
                            onClick={() => setShowFilterDropdown(!showFilterDropdown)}
                        >
                            <span className="truncate">{finding_tag || "Filter by category..."}</span>
                            <svg 
                                className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 flex-shrink-0 ml-2 ${showFilterDropdown ? 'rotate-180' : ''}`} 
                                fill="none" 
                                stroke="currentColor" 
                                viewBox="0 0 24 24"
                            >
                            ...
                            <div className="absolute top-full left-0 right-0 mt-2 bg-background-400 border border-gray-600 rounded-lg shadow-xl z-50 max-h-96 overflow-y-auto">
                                {Object.entries(availableCategories).map(([category, tags]) => (
                                    <div key={category} className="border-b border-gray-600 last:border-b-0">
                                        <div className="px-3 sm:px-4 py-2 bg-background-500 text-purple-400 font-medium text-xs sm:text-sm">
                                            {category}
                                        </div>
                                        {tags.map((tag) => (
                                            <button
                                                key={tag}
                                                className={`w-full text-left px-3 sm:px-4 py-2 text-xs sm:text-sm hover:bg-background-300 transition-colors duration-200 ${
                    ...
                    {finding_tag && (
                        <div className="text-center mt-3 sm:mt-4">
                            <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1 sm:py-2 bg-purple-500/20 border border-purple-500/30 rounded-full">
                                <span className="text-purple-400 text-xs sm:text-sm">Filtered by: {finding_tag}</span>
                                <button 
                                    onClick={() => setFinding_tag("")}
                            ...
                            <div className="relative glass rounded-2xl overflow-hidden hover:shadow-glow transition-all duration-300 transform hover:scale-105 h-full flex flex-col">
                                ...
                                <div className="relative h-40 sm:h-48 overflow-hidden flex-shrink-0">
                                    ...
                                    <div className="absolute bottom-3 sm:bottom-4 left-3 sm:left-4 right-3 sm:right-4">
                                        <h3 className="font-family_header2 text-lg sm:text-xl font-bold text-white mb-1 sm:mb-2">
                                            {project.name}
                                        </h3>
                                    </div>
                                </div>
                                ...
                                <div className="p-4 sm:p-6 flex flex-col flex-1">
                                    ...
                                    <div className="flex flex-wrap gap-1 sm:gap-2 mb-3 sm:mb-4">
                                        {project.techs.map((tech, techIndex) => (
                                            <span 
                                                key={techIndex}
                                                className="px-2 sm:px-3 py-1 bg-background-300 rounded-full text-xs font-medium text-gray-300 border border-gray-600"
                                            >
                                                {tech}
                                            </span>
                                        ))}
                                    ...
                                    <div className='flex gap-2 sm:gap-3 mt-auto'>
                                        {project.github_link && (
                                            <a 
                                                ...
                                                className="flex-1 flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 bg-background-300 rounded-lg text-gray-300 hover:bg-background-100 hover:text-white transition-all duration-300 border border-gray-600 hover:border-gray-500 text-xs sm:text-sm"
                                            >
                                        ...
                                        {project.demo_link && (
                                            <a 
                                                ...
                                                target="_blank" 
                                                rel="noopener noreferrer"
                                                className="flex-1 flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg text-white hover:from-purple-600 hover:to-pink-600 transition-all duration-300 transform hover:scale-105 text-xs sm:text-sm"
                                            >
                                                <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {filteredProjects.length > initialShowCount && (
                    <div className="text-center mt-8 sm:mt-12">
                        <button
                            ...
                            className="group px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold rounded-xl transition-all duration-300 transform hover:scale-105 shadow-glow hover:shadow-lg text-sm sm:text-base"
                        >
                ...
                <div className='mt-12 sm:mt-16 text-center'>
                    <h3 className="gradient-text font-family_header2 text-2xl sm:text-3xl font-bold mb-3 sm:mb-4">
